      ...
      <div className="flex">
        ...
        <div className="flex-1 p-5 md:p-6">
          ...
          {rows.map((row) => (
            <div
              key={row.name}
              className="grid grid-cols-12 gap-3 items-center py-3.5 border-b border-gray-50 dark:border-white/[0.04] px-1 last:border-0"
            >
                ...
                <span
                  className={`inline-block px-2 py-0.5 text-xs rounded-full font-medium ${
                    row.status === 'Completed'
                      ...
                      : 'bg-gray-100 dark:bg-white/[0.08] text-gray-500 dark:text-neutral-500'
                  }`}
                >
                  {row.status}
                </span>
              ...
              <div className="col-span-2 hidden sm:block">
                <span className={`text-xs font-medium ${
                  row.priority === 'High' ? 'text-amber-600 dark:text-amber-400' : row.priority === 'Medium' ? 'text-gray-500 dark:text-neutral-500' : 'text-gray-400 dark:text-gray-500'
                }`}>
                  {row.priority}
                </span>
              </div>
      ...
      <div className="flex items-center justify-between mb-8">
        {steps.map((step, i) => (
          <React.Fragment key={step.label}>
            ...
            {i < steps.length - 1 && (
              <div className={`flex-1 h-px mx-2 ${step.done ? 'bg-emerald-300 dark:bg-emerald-600' : 'bg-gray-200 dark:bg-white/[0.06]'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={task.task} className="flex items-center space-x-3">
              ...
              <span className={`text-sm ${task.done ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-700 dark:text-neutral-400'}`}>
                {task.task}
              </span>
            </div>
          ))}
...
function DesignSystemMockup() {
  return (
    <div className="rounded-xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] p-6 md:p-8 shadow-sm">
      ...
      <div className="border-t border-gray-100 dark:border-white/[0.06] pt-5 mt-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-baseline space-x-4">
            ...
            <span className="text-sm text-gray-600 dark:text-neutral-500">Standard paragraph text</span>
          </div>
          <div className="flex items-baseline space-x-4">
